...
import { TASK_DEPOSIT_REBALANCE_WITHDRAW } from "./task-names";

task(TASK_DEPOSIT_REBALANCE_WITHDRAW, "Prints the list of accounts", async (_taskArgs, hre) => {
    ...
    const canTake = await deployed["balanceOf(address)"](deployer)
    console.log("canTake", canTake.toString())
    console.log("canTake", hre.ethers.utils.formatEther(canTake))


    console.log("Withdrawing from Contract Withdrawing from Aave")
    await (await deployed.withdraw(canTake, { gasLimit: 5000000 })).wait()
